...
app.get('/v3/sections/discord_servers', async (req, res) => {
    var inviteINK = req.query.link as String;
    if (inviteINK) {
    ...
    try {
        const response = await axios.get(`https://discord.com/api/v10/invites/${inviteINK}`, {
          headers: {
